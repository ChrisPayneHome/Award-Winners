...
final_data <- oscar_data %>%
  ...
  # join the bafta data
  left_join(bafta_data, by = "Year", suffix = c("", "_bafta")) %>%
  mutate(bafta_nominated = ifelse(Film == Film_bafta & !Won_bafta, TRUE, FALSE),
         bafta_won = ifelse(Film == Film_bafta & Won_bafta, TRUE, FALSE)) %>%
  select(-Won_bafta,
         -Film_bafta)
